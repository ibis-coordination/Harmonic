...
1. They act through the studio's proxy user
...
3. Sets session cookies: `proxy_user_id`, `representation_session_id`
...
- `current_user` returns the proxy user (not the person)
- Actions are attributed to the proxy user
...
The record shows:
- Who represented (the person, not the proxy)
...
| `proxy_user_id` | uuid | The studio's proxy user |
