...
1. If the agent has a **funding pool**, draw uniformly at random from the pool's actively-enrolled members who are still active members of the pool's collective, have an active billing customer, pass the balance gate, and are under the pool's draw ceiling. Random-per-call is deliberate: memoryless selection needs no ledger consultation for fairness, no enrollment-time catch-up, and no locking; cost spreads evenly over time.
...
**Funding pools** (`FundingPool`, one per standard collective) are the common-pool mechanism: a collective's members pool their own prepaid balances to fund the collective's agents. The feature is gated by the **operator-managed `funding_pools` collective-level flag** (`operator_managed: true` in `config/feature_flags.yml`): a platform admin enables it per hand-picked collective (`collective.enable_feature_flag!("funding_pools")`, plus once at tenant level) — it is never self-serve from collective settings, and the self-serve flag form skips operator-managed flags. The flag is also the kill switch: turning it off suspends the pool's draws immediately (`funding_collective_unavailable`) and pauses enrollment/attachment, while withdrawal and detachment (the exits) keep working. Collective membership is free and social; the billing consent lives on an explicit **enrollment** (`FundingPoolEnrollment`) — self-serve enroll and withdraw in the collective's settings, gated on funded billing (active customer + prepaid-credit subscription) at enrollment and re-checked per call at draw time. Lapsed or dry members are skipped in draws; withdrawal (archives the row — it's the consent record for past draws) drops the member from the next draw. Admins open/close the pool and attach/detach agents from settings; an agent can be attached only while its principal is actively enrolled (`users.funding_pool_id`, validated at attach and re-checked per call — `no_primary` if the principal withdraws or leaves). Closing the pool (or archiving the collective) suspends its agents from the next call: 1-to-1 agent↔funding-source, no fallback payer. The pool never holds funds: each call bills exactly one member's own Stripe balance.

The funding *relationship* is public: every funded agent's profile shows "Funded by ⟨collective⟩" with a link — part of the agent's accountability story. The enrolled members are not listed on the agent's profile; the pool's roster has ordinary member-only visibility on the collective's settings page.
...
- `funding_pools.member_daily_draw_cap_cents` — the pool's ceiling on what its agents may bill any single enrolled member per day; a member at the ceiling is skipped in draws (draws by *other* pools don't count against it).
