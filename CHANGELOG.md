...
- **Token cleanup unblocked: token references nullify on delete** (#557) — `CleanupExpiredTokensJob` had failed on every run since 2026-08-24 (Sentry RUBY-RAILS-1A, 505 events): its single `delete_all` batch aborted on the first token still referenced by a `harmonic_bridge_setups` row, so no expired tokens were cleaned at all. The three FKs referencing `api_tokens` without an `ON DELETE` action (`harmonic_bridge_setups.api_token_id`, `.llm_api_token_id`, `llm_usage_records.api_token_id`) now `SET NULL`, matching the `mcp_tool_call_logs` precedent — historical records outlive their token with a nullified reference. Deploy: web, one fast migration; the job's next run drains ~3 months of backlog.

### Changed

- **Memory-pressure hardening after the 2026-09-15 droplet lockup** (#558, #559) — post-mortem of four OOM events found no single leak: a structurally overcommitted 4GB host (clamd's ~950MB signature DB resident, Ruby RSS growing ~35% over days of uptime, clamd's daily signature reload double-buffering ~1GB as the likely tipping trigger). Compose: `MALLOC_ARENA_MAX=2` on web + sidekiq, a 1536M memory limit on clamav (the only uncapped app container — future spikes restart it alone instead of host-wide OOM), and a mounted `config/clamav/clamd.conf` with `ConcurrentDatabaseReload no` (reload in place; brief scan pause, attachment scanning is fail-open). Host: `scripts/provision-host.sh` — idempotent provisioning (4G swapfile + `vm.swappiness=10`) so a rebuilt droplet converges to identical config; the DO memory alert (85%/5m) is captured in `docs/DEPLOYMENT.md` as a repeatable `doctl` command. Deploy: compose-only recreate of web/sidekiq/clamav; `git pull` in the deployment directory delivers the mounted config (swap already live on prod since 2026-09-15).
- **Dependency bumps** — mail 2.9.0 → 2.9.1 + net-imap 0.6.4.1 → 0.6.6 (#555), agent-runner vitest → 4.1.11 (#556).
...
- **Deleting an automation rule no longer destroys its history — or 500s** (#528) — deleting a bridge-connected notification webhook crashed on a foreign key from the bridge-setup record, and deleting any rule destroyed its run history. Rule deletion is now a soft delete via the new `HasDeletedAt` concern (also adopted by `SoftDeletable` and API tokens): the rule stops firing, leaves listings and billing, and frees the one-webhook-per-user slot, while its runs, task-run attribution, and bridge-setup references survive. Deleting a webhook now also syncs the Stripe subscription quantity down. Deploy: web, one additive migration.
- **Sprite setup: login actually happens, and each agent gets its own sprite** (#528) — harmonic-bridge 0.2.2 fixes `setup-sprite` skipping the Claude Code login (the base image ships `~/.claude` without credentials, fooling the auth check) and hands off to `claude /login` (`claude login` is not a real subcommand). The bridge-setup page command now includes `--sprite-name harmonic-<handle>`, so each agent gets its own sprite instead of all sharing one named "harmonic-agent". Publish 0.2.2 by tagging `bridge-v0.2.2`.
...
- **Sys-admin task run pages show steps for runs on other tenants** (#511) — the admin agent-runner dashboard only runs on the primary tenant, but task runs can belong to any tenant (in production the Trio personas run on the sandbox tenant). Steps were loaded through a tenant-scoped association, which silently filtered another tenant's steps to empty — the page showed a positive steps count and then "No step data recorded." Steps now load admin-scoped; system-agent runs render their timelines unredacted cross-tenant, as intended.

## [1.51.0] - 2026-07-18

### Added

- **Trio: the Melody, Counterpoint, and Cadence built-in agents** (#510) — "Trio" now names the feature: an ensemble of three built-in AI agents enabled as a set by the existing `trio` flag, private workspaces included. **Melody** focuses on doing (expert in Harmonic's features and controls, holds `automator`); **Counterpoint** on verifying (Harmonic's rules and boundaries, `moderator`); **Cadence** on learning (Harmonic's information architecture, `summarizer`). The agent formerly named Trio is retired in place by migration — roles stripped, memberships archived, automation rules disabled, User rows kept so authored content keeps attribution — and Cadence is net-new like its siblings. All three are instances of one pattern: a `Personas` registry holds each persona's identity facts and prompt, `PersonaSeeder`/`PersonaActivator` manage the set (reconcile heals partial ensembles), and every active persona holds three activator-managed roles — its persona role, the shared `trio` ensemble role (`@trio` fans out to every enabled persona: up to three replies per mention), and its capability role. New `/help/trio` page; agents are referred to as "they", never "it", across user-facing copy. Deploy with migrations: **retire the prod external melody agent first** (its `melody` handle trips the migration's namespace guard by design), replace `TRIO_DEFAULT_MODEL` with `CADENCE_DEFAULT_MODEL`/`MELODY_DEFAULT_MODEL`/`COUNTERPOINT_DEFAULT_MODEL`, and expect trio-enabled collectives to gain Melody and Counterpoint on their next settings-save reconcile.
- **Self-serve funding pools fund the built-in agents** (#504) — a paid-tier collective can now open a funding pool without an operator flag, and the pool automatically funds the collective's built-in agents, whose principal is now the collective's own identity user (backfilled by migration). Pool-funding any other agent stays operator-gated — the trust line: a collective-principaled agent on Harmonic's own runtime doing the collective's work is what the pool is for; an individually-principaled agent spending everyone's balances is not. On billing tenants built-in agents route exclusively through the LLM gateway (the LiteLLM special case is gone); without an open pool their dispatch fails fast with a message pointing at collective settings. Availability is checked per draw, so tier lapse suspends pool spending immediately and re-upgrade resumes it. Also fixes the agent-runner to send `max_completion_tokens` for `openai/` models. Deploy with migrations.
- **Automator and moderator collective roles** (#505) — two new grantable member roles, assignable to anyone (human or agent) through the existing role-grant flow. `automator` grants the full automation-management surface admins have (the paid-tier gate on automations itself is unchanged); `moderator` grants no capabilities yet — the moderation controls it will gate are a separate track, but naming the role now lets it be granted, displayed, and mentioned. Group tags (`@automators`/`@moderators`), the members-page picker, validation, and role-granted notifications all derive from the role list automatically.
- **Draw-authorization receipts on the LLM usage ledger** (#502) — every pool draw now snapshots, at payer-selection time, the enrollment id and both ceilings it was authorized against (the member's enrollment ceiling and the pool's per-member ceiling), making each ledger row a self-contained receipt: a dispute over whether the pool drew a member's credits fairly is settleable from the ledger alone, without trusting mutable enrollment rows. Forward-only — the historical terms were never recorded, so there is nothing honest to backfill. Deploy with migrations.
- **Public-writes state in agent-readable settings** (#495) — the agent-facing markdown settings view now shows whether public writes are enabled, driven by the same predicate the write gate enforces so the display can't drift, with the observable consequences documented when off. Previously the permission was invisible until a public write failed at submit time.

### Changed

- **Persona handles and mention resolution restructured** (#508) — a collective's built-in agent is no longer the shared `/u/trio` identity: the persona role on the collective-member row is the single source of truth (`collectives.trio_user_id` dropped), handles follow `trio-[collective_handle]` and link to the agent's own profile (collective renames cascade), and `@trio` mentions resolve through the role. The `trio`/`trio-*` handle namespace is reserved unconditionally — for collectives too, since every collective mints an identity user whose handle mirrors its own. The migration backfills roles, renames legacy handles, and fail-fasts with the offending `(tenant_id, handle)` pairs if any non-trio user or collective already holds a reserved handle. Deploy with migrations.
- **Funding pool page reworked around usage tables and a hard upper bound** (#501) — enrolled members and funded agents render as tables (ceiling, max possible per 30 days with the arithmetic in a hover tooltip, actual last-30-days spend), and a new "Maximum Possible Draw" section states the member-choice-independent bound — `pool ceiling × member count`, with the arithmetic shown — since the pool's ceiling binds every member regardless of personal ceilings. Also fixes the agents-table total truncating fractional cents.
- **CI: controller tests sharded into two parallel jobs** (#507) — deterministic round-robin over sorted filenames; the controllers group was the slowest matrix entry, often more than double any other.
