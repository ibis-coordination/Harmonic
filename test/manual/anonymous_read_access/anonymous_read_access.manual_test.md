...
3. Repeat for the private tenant's decision, commitment, and `/help` URL.
...
5. The top-right header should show a "Log in" button — NOT a notification
...
- [x] A. Anon can read the three item URLs and help pages on the public tenant
      — verified via curl on `/n/68e4986c`, `/d/aec74da4`, `/c/2761e577`,
      `/help`, `/help/privacy` (all 200)
- [x] B. All four URLs redirect to /login on a private tenant
      — verified on `second.harmonic.local/help` → 302
- [x] C. Edit/Pin/Report/Vote/Join/Comment-form hidden for anon, login CTAs in their place
      — "Log in to participate" on /d/, "Log in to rsvp this commitment" on
      /c/ (calendar event), "Log in to comment" on /d/ and /c/, "Log in to
      confirm reading this note" on /n/. Zero Edit/Pin/Report buttons in
      anon response bodies.
...
- [x] F. Markdown returns 200; JSON returns 401
      — `.md` returned `text/markdown` for all three; JSON returned 401
