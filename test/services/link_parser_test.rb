...
class LinkParserTest < ActiveSupport::TestCase
  ...
  test "parse extracts note links from text" do
    ...
    text = "Check out https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/n/#{note.truncated_id} for details."
  ...
  test "parse extracts decision links from text" do
    ...
    text = "Vote here: https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/d/#{decision.truncated_id}"
  ...
  test "parse extracts commitment links from text" do
    ...
    text = "Join us: https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/c/#{commitment.truncated_id}"
  ...
  test "parse extracts multiple links from text" do
    ...
    text = <<~TEXT
      Check the note: https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/n/#{note.truncated_id}
      And vote here: https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/d/#{decision.truncated_id}
    TEXT
  ...
  test "parse does not duplicate records when same link appears multiple times" do
    note = create_note(tenant: @tenant, superagent: @superagent, created_by: @user)
    link = "https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/n/#{note.truncated_id}"
    text = "First: #{link}\nSecond: #{link}\nThird: #{link}"
  ...
  test "parse ignores links from different subdomains" do
    note = create_note(tenant: @tenant, superagent: @superagent, created_by: @user)
    text = "Check out https://other-tenant.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/n/#{note.truncated_id}"
  ...
  test "parse handles full UUIDs" do
    note = create_note(tenant: @tenant, superagent: @superagent, created_by: @user)
    text = "Check out https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/n/#{note.id}"
  ...
  test "parse handles scene URLs" do
    ...
    text = "Check https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/scenes/#{@superagent.handle}/n/#{note.truncated_id}"
  ...
  test "parse_path extracts record from path" do
    note = create_note(tenant: @tenant, superagent: @superagent, created_by: @user)
    path = "/studios/#{@superagent.handle}/n/#{note.truncated_id}"
  ...
  test "parse_path returns nil for invalid path" do
    record = LinkParser.parse_path("/studios/#{@superagent.handle}/n/nonexistent-id")
    assert_nil record
  end

  test "parse_path handles full UUID" do
    note = create_note(tenant: @tenant, superagent: @superagent, created_by: @user)
    path = "/studios/#{@superagent.handle}/n/#{note.id}"
  ...
  test "can be initialized with subdomain and studio_handle" do
    parser = LinkParser.new(subdomain: @tenant.subdomain, superagent_handle: @superagent.handle)
    assert_not_nil parser
  end
  ...
  test "instance parse extracts links from record text" do
    note1 = create_note(tenant: @tenant, superagent: @superagent, created_by: @user, title: "Note 1", text: "First note")
    note2 = create_note(
      tenant: @tenant,
      superagent: @superagent,
      created_by: @user,
      title: "Note 2",
      text: "References https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/n/#{note1.truncated_id}"
    )
  ...
  test "instance parse with subdomain requires text" do
    parser = LinkParser.new(subdomain: @tenant.subdomain, superagent_handle: @superagent.handle)
    note = create_note(tenant: @tenant, superagent: @superagent, created_by: @user)
    text = "See https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/n/#{note.truncated_id}"
  ...
  test "parse_and_create_link_records creates Link records" do
    ...
    new_text = "References https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/n/#{note1.truncated_id}"
  ...
  test "parse_and_create_link_records works with decision description" do
    note = create_note(tenant: @tenant, superagent: @superagent, created_by: @user, title: "Reference Note", text: "Content")
    ...
    new_desc = "See https://#{@tenant.subdomain}.#{ENV['HOSTNAME']}/studios/#{@superagent.handle}/n/#{note.truncated_id}"
