...
class AgentRunnerDispatchServiceTest < ActiveSupport::TestCase
  ...
  test "fails task when credit balance is zero" do
    ...
    @task_run.reload
    assert_equal "failed", @task_run.status
    assert_includes @task_run.error, "Insufficient prepaid balance"
  end
  ...
  test "a chat turn for a pool-funded agent fails fast at dispatch" do
    # Pool funding doesn't cover 1-on-1 chat; failing here (not at the
    ...
    @task_run.reload
    assert_equal "failed", @task_run.status
    assert_match(%r{pool funding doesn't cover 1-on-1 chat}i, @task_run.error)
  end
