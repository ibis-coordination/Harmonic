...
class AuditChainVerificationTest < ActionDispatch::IntegrationTest
  ...
  test "verify.json contains all audit chain fields with correct types" do
    ...
    chain.each_with_index do |entry, i|
      ...
      assert_equal 3, entry["schema_version"], "entry #{i}: schema_version should be 3 for new entries"
      ...
      assert entry["representative_id"].is_a?(String), "entry #{i}: representative_id should be string"
      assert entry["representative_handle"].is_a?(String), "entry #{i}: representative_handle should be string"
      assert entry["representative_token"].is_a?(String), "entry #{i}: representative_token should be string"
      assert entry["representative_token_salt"].is_a?(String), "entry #{i}: representative_token_salt should be string"
      assert entry["representation_kind"].is_a?(String), "entry #{i}: representation_kind should be string"
      assert_equal "", entry["representative_token"], "entry #{i}: direct action must have empty representative_token"
      assert_equal "", entry["representation_kind"], "entry #{i}: direct action must have empty representation_kind"
