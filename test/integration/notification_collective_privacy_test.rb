...
class NotificationCollectivePrivacyTest < ActionDispatch::IntegrationTest
  # Tests for collective privacy in the notification system.
  #
  # These tests verify that notifications respect collective membership boundaries:
  # users should only receive notifications about content from collectives they
  # are members of.
  #
  # Scenario:
  # - User A is a member of Collective X
  # - User B is NOT a member of Collective X
  # - User A creates content in Collective X and @mentions User B
  #
  # Expected behavior: No notification should be created for User B
  # because they are not a member of Collective X and cannot see its content.
  #
  # Bug #1: NotificationDispatcher.handle_note_event notifies all mentioned users
  # without checking collective membership. The fix should filter out mentioned users
  # who are not members of the collective where the content was created.
  #
  # This bug affects:
  # - Notes with @mentions
  ...
  def setup
    ...
    # Collective X - only User A is a member (this is the private collective)
    @collective_x = create_collective(tenant: @tenant, created_by: @user_a, name: "Collective X", handle: "collective-x")
    @collective_x.add_user!(@user_a)
    # Note: User B is intentionally NOT added to collective_x

    host! "#{@tenant.subdomain}.#{ENV["HOSTNAME"]}"
  end

  test "non-member should NOT receive notification when mentioned in private collective" do
    # Set up the collective scope for creating the note
    Collective.scope_thread_to_collective(subdomain: @tenant.subdomain, handle: @collective_x.handle)

    # User A creates a note that mentions User B (who is NOT a collective member)
    secret_content = "This is private collective content that @user_b should NOT see!"
    note = create_note(
      tenant: @tenant,
    ...
    # Verify User B is NOT a member of Collective X
    refute @collective_x.user_is_member?(@user_b), "Test setup error: User B should NOT be a member of Collective X"

    # User B should NOT receive a notification because they are not a member of Collective X
    notification_for_user_b = NotificationRecipient.unscoped.find_by(user: @user_b)

    assert_nil notification_for_user_b,
      "Non-members should NOT receive notifications about private collective content"
  end
  ...
  test "non-member should NOT see private content on notifications page" do
    ...
    # Sign in as User B (who is NOT a member of Collective X)
    ...
    # User B should NOT see any private collective content on their notifications page
    assert_no_match(/CONFIDENTIAL/, response.body,
      "Non-members should NOT see private collective content on their notifications page")
  ...
  test "member should receive notification when mentioned" do
    ...
    # User A creates a note that mentions User C (who IS a collective member)
    note = create_note(
      tenant: @tenant,
      collective: @collective_x,
      created_by: @user_a,
    ...
    assert @collective_x.user_is_member?(user_c), "Test setup error: User C should be a member of Collective X"
  ...
  test "non-member should NOT receive notification when mentioned in comment on decision" do
    ...
    # User A creates a decision in Collective X
    ...
    # User A adds a comment that mentions User B (who is NOT a collective member)
    comment = Note.create!(
      tenant: @tenant,
      collective: @collective_x,
      created_by: @user_a,
    ...
    assert_nil notification_for_user_b,
      "Non-members should NOT receive notifications about comments in private collectives"
  end

  test "non-member should NOT receive notification when mentioned in comment on commitment" do
    Collective.scope_thread_to_collective(subdomain: @tenant.subdomain, handle: @collective_x.handle)

    # User A creates a commitment in Collective X
    ...
    assert_nil notification_for_user_b,
      "Non-members should NOT receive notifications about comments on commitments in private collectives"
  end

  test "non-member should NOT receive notification when mentioned in comment on note" do
    Collective.scope_thread_to_collective(subdomain: @tenant.subdomain, handle: @collective_x.handle)

    # User A creates a note in Collective X
    ...
    assert_nil notification_for_user_b,
      "Non-members should NOT receive notifications about comments on notes in private collectives"
  end
  ...
  # also checking collective membership.
  ...
  test "non-member should NOT receive notification when mentioned in decision description" do
    ...
    assert_nil notification_for_user_b,
      "Non-members should NOT receive notifications about decisions in private collectives"
  end
  ...
  test "non-member should NOT receive notification when mentioned in commitment description" do
    ...
    assert_nil notification_for_user_b,
      "Non-members should NOT receive notifications about commitments in private collectives"
  end
  ...
  test "member should receive notification when mentioned in decision description" do
    ...
    # Verify User C IS a member of Collective X
    assert @collective_x.user_is_member?(user_c), "Test setup error: User C should be a member of Collective X"
  ...
  test "non-member should NOT receive notification when mentioned in option title" do
    ...
    assert_nil notification_for_user_b,
      "Non-members should NOT receive notifications about options in private collectives"
  end
  ...
  test "non-member should NOT receive notification when mentioned in option description" do
    ...
    assert_nil notification_for_user_b,
      "Non-members should NOT receive notifications about option descriptions in private collectives"
  end
