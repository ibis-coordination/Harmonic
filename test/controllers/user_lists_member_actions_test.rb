...
# Tests for /lists/:id/actions/add_member_to_list and /lists/:id/actions/remove_member_from_list.
class UserListsMemberActionsTest < ActionDispatch::IntegrationTest
  ...
  def add_member_to_list!(list, headers:, user_handle:)
    post "/lists/#{list.truncated_id}/actions/add_member_to_list",
      params: { user_handle: user_handle }.to_json, headers: headers
  end

  def remove_member_from_list!(list, headers:, user_handle:)
    post "/lists/#{list.truncated_id}/actions/remove_member_from_list",
      params: { user_handle: user_handle }.to_json, headers: headers
  end
  ...
  test "describe_add_member returns the description for the owner" do
    ...
    get "/lists/#{list.truncated_id}/actions/add_member_to_list", headers: @owner_h
    assert_response :success
    assert_includes response.body, "add_member_to_list"
    assert_includes response.body, "user_handle"
  end

  test "describe_remove_member returns the description for the owner" do
    list = list_with(add_policy: "owner_only")
    get "/lists/#{list.truncated_id}/actions/remove_member_from_list", headers: @owner_h
    assert_response :success
  ...
  test "owner_only: owner can add another user" do
    ...
    assert_difference -> { list.user_list_members.count }, +1 do
      add_member_to_list!(list, headers: @owner_h, user_handle: handle_of(@other))
    end
    assert_response :success
  end
  ...
  test "owner_only: non-owner cannot add (returns 403)" do
    ...
    assert_no_difference -> { list.user_list_members.count } do
      add_member_to_list!(list, headers: @other_h, user_handle: handle_of(@third))
    end
  ...
  test "owner_only: non-owner cannot self-add (returns 403)" do
    list = list_with(add_policy: "owner_only")
    add_member_to_list!(list, headers: @other_h, user_handle: handle_of(@other))
    assert_response :forbidden
  end
  ...
  test "members_add: a member can add another user" do
    ...
    assert_difference -> { list.user_list_members.count }, +1 do
      add_member_to_list!(list, headers: @member_h, user_handle: handle_of(@other))
    end
    assert_response :success
  end
  ...
  test "anyone_add: a collective non-member is rejected by member_is_collective_member (422)" do
    ...
    add_member_to_list!(list, headers: @other_h, user_handle: handle_of(stranger))
  ...
  test "add_member_to_list rejects when actor has blocked target (422)" do
    list = list_with(add_policy: "anyone_add")
  ...
  test "add_member_to_list rejects when target has blocked actor (symmetric, 422)" do
    list = list_with(add_policy: "anyone_add")
  ...
  test "add_member_to_list rejects when owner has blocked target, even with anyone_add" do
    list = list_with(add_policy: "anyone_add")
  ...
  test "add_member_to_list is idempotent — adding an existing member succeeds without dupes" do
  ...
  test "add_member_to_list 404s for a list the actor cannot see (private + not owner)" do
    list = UserList.create!(creator: @owner, owner: @owner, name: "Hidden", visibility: "private")
    add_member_to_list!(list, headers: @other_h, user_handle: handle_of(@third))
    assert_response :not_found
  end

  test "add_member_to_list 422s for an unknown user_handle" do
    list = list_with(add_policy: "owner_only")
    post "/lists/#{list.truncated_id}/actions/add_member_to_list",
  ...
  test "add_member_to_list 422s when user_handle resolves only in a different tenant" do
    list = list_with(add_policy: "anyone_add")
  ...
  test "add_member_to_list on a primary list rejects non-owner (owner_only enforced)" do
    primary = @owner.primary_user_list_in!(@tenant)
    add_member_to_list!(primary, headers: @other_h, user_handle: handle_of(@third))
    assert_response :forbidden
  end
  ...
  test "remove_member_from_list: owner can remove anyone" do
    list = list_with(add_policy: "owner_only", members: [@other])
    assert_difference -> { list.user_list_members.count }, -1 do
      remove_member_from_list!(list, headers: @owner_h, user_handle: handle_of(@other))
    end
    assert_response :success
  end

  test "remove_member_from_list: a user can remove themselves" do
    list = list_with(add_policy: "owner_only", members: [@other])
    assert_difference -> { list.user_list_members.count }, -1 do
      remove_member_from_list!(list, headers: @other_h, user_handle: handle_of(@other))
    end
    assert_response :success
  end

  test "remove_member_from_list: a non-owner cannot remove someone else (403)" do
    list = list_with(add_policy: "anyone_add", members: [@other, @third])
    remove_member_from_list!(list, headers: @other_h, user_handle: handle_of(@third))
    assert_response :forbidden
  end

  test "remove_member_from_list: members policy doesn't grant member-removes-others" do
    ...
    remove_member_from_list!(list, headers: @member_h, user_handle: handle_of(@other))
    assert_response :forbidden
  end

  test "remove_member_from_list: self-removal is allowed even when actor↔target are blocked" do
    list = list_with(add_policy: "owner_only", members: [@other])
  ...
  test "remove_member_from_list: removing a non-member reports 'Not on this list.'" do
    list = list_with(add_policy: "owner_only")
    assert_no_difference -> { list.user_list_members.count } do
      remove_member_from_list!(list, headers: @owner_h, user_handle: handle_of(@other))
    end
  ...
  test "actions_index_show offers add_member_to_list to owner on owner_only" do
    list = list_with(add_policy: "owner_only")
  ...
  test "actions_index_show hides add_member_to_list from non-owner on owner_only" do
    list = list_with(add_policy: "owner_only")
    get "/lists/#{list.truncated_id}/actions", headers: @other_h
    assert_response :success
    assert_not_includes response.body, "add_member_to_list"
  end

  test "actions_index_show offers add_member_to_list to any viewer on anyone_add" do
    list = list_with(add_policy: "anyone_add")
    get "/lists/#{list.truncated_id}/actions", headers: @other_h
    assert_response :success
    assert_includes response.body, "add_member_to_list"
  ...
  test "actions_index_show offers add_member_to_list to a list member on members_add but hides it from non-members" do
    list = list_with(add_policy: "members_add", members: [@member])
  ...
  test "actions_index_show offers add_member_to_list to any viewer on self_add (they can add themselves)" do
    list = list_with(add_policy: "self_add")
    get "/lists/#{list.truncated_id}/actions", headers: @other_h
    assert_includes response.body, "add_member_to_list"
  end

  test "actions_index_show offers remove_member_from_list only to owner or self" do
    list = list_with(add_policy: "anyone_add", members: [@other])
    # owner sees it
    get "/lists/#{list.truncated_id}/actions", headers: @owner_h
    assert_includes response.body, "remove_member_from_list"
  ...
  # ============================================================
  # /lists/:id/actions/join_list — markdown self-join (no params)
  # ============================================================

  test "join: self_add list adds the actor with no params" do
    list = list_with(add_policy: "self_add")
  ...
  test "join: owner_only list rejects a non-owner with 403" do
    list = list_with(add_policy: "owner_only")
    post "/lists/#{list.truncated_id}/actions/join_list", params: "{}", headers: @other_h
    assert_response :forbidden
  ...
  test "join: members_add list rejects a non-member with 403" do
    list = list_with(add_policy: "members_add")
    post "/lists/#{list.truncated_id}/actions/join_list", params: "{}", headers: @other_h
    assert_response :forbidden
    assert_not list.user_list_members.exists?(user_id: @other.id)
  ...
  test "describe_join_list returns the action description" do
    list = list_with(add_policy: "self_add")
    get "/lists/#{list.truncated_id}/actions/join_list", headers: @other_h
    assert_response :success
    assert_includes response.body, "join_list"
  end

  test "frontmatter on a self_add list lists `join` for an eligible non-member" do
    list = list_with(add_policy: "self_add")
    get "/lists/#{list.truncated_id}", headers: @other_h
    assert_response :success
    frontmatter = response.body.split("---").at(1).to_s
    assert_includes frontmatter, "name: join_list"
  end

  test "frontmatter omits `join` for a non-eligible viewer (owner_only, non-owner)" do
    list = list_with(add_policy: "owner_only")
    get "/lists/#{list.truncated_id}", headers: @other_h
    assert_response :success
    frontmatter = response.body.split("---").at(1).to_s
    assert_not_includes frontmatter, "name: join_list"
  end

  test "frontmatter omits `join` when the viewer is already a member" do
    list = list_with(add_policy: "self_add", members: [@other])
    get "/lists/#{list.truncated_id}", headers: @other_h
    assert_response :success
    frontmatter = response.body.split("---").at(1).to_s
