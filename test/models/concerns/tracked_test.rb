...
class TrackedTest < ActiveSupport::TestCase
  test "creating a note creates a note.created event" do
    ...
    assert_difference "Event.count", 1 do
      create_note(tenant: tenant, collective: collective, created_by: user, text: "Test note")
    end
  ...
  test "updating a note creates a note.updated event" do
    ...
    note = create_note(tenant: tenant, collective: collective, created_by: user, text: "Original text")
  ...
  test "updating only updated_at does not create an event" do
    ...
    note = create_note(tenant: tenant, collective: collective, created_by: user, text: "Test")
  ...
  test "deleting a note creates a note.deleted event" do
    ...
    note = create_note(tenant: tenant, collective: collective, created_by: user, text: "Test note")
    note_id = note.id
  ...
  test "event metadata includes truncated_id and text" do
    ...
    note = create_note(tenant: tenant, collective: collective, created_by: user, text: "Hello world")
