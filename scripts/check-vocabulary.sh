...
PROSE_PATTERN='visibility (zone|level)|levels of visibility|\bpublic spaces\b|\bmain space\b|\bmembers-only space\b|\bmain collective\b|\btenants?\b|\bprivate chats?\b|agent'"'"'?s owner|owner of this (ai_)?agent|\bprimary list\b|\buser list\b|\bpersonas?\b|\busernames?\b|spend(ing)? limits?|\bquotas?\b|direct messages?|\bDMs?\b|prepaid credits?|credit balance|\bcredits\b|trustee grants?|activity logs?'
