...
class AutomationRuleRunResource < ApplicationRecord
  extend T::Sig
  ...
  # resources that may belong to different superagents than where the automation run started
  def self.default_scope
    where(tenant_id: Tenant.current_id)
  end

  # Find the automation run that created a given resource (if any)
  # Returns nil if the resource was not created by an automation rule run
  sig { params(resource: T.untyped).returns(T.nilable(AutomationRuleRun)) }
  def self.run_for(resource)
  ...
  def set_tenant_id
    ...
    self.tenant_id = T.must(automation_rule_run&.tenant_id || Tenant.current_id)
  end
