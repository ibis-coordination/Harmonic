import { Controller } from "@hotwired/stimulus"
import hljs from "highlight.js/lib/core"
import python from "highlight.js/lib/languages/python"
// CSS theme loaded via Sprockets (app/assets/stylesheets/hljs-github.css)

hljs.registerLanguage("python", python)

/**
 * Adds syntax highlighting and a copy button to <pre><code> blocks.
 *
 * Usage:
 *   <div data-controller="code-block" data-code-block-language-value="python">
 *     <pre><code>...</code></pre>
 *   </div>
 */
export default class CodeBlockController extends Controller {
  static values = { language: { type: String, default: "" } }
  declare readonly languageValue: string

  connect(): void {
    const code = this.element.querySelector("code")
    if (!code) return

    if (this.languageValue) {
      code.classList.add(`language-${this.languageValue}`)
      hljs.highlightElement(code)
    }

    this.addCopyButton(code)
  }

  private addCopyButton(code: HTMLElement): void {
    const pre = code.parentElement
    if (!pre) return

    const wrapper = document.createElement("div")
    wrapper.style.position = "relative"
    pre.parentNode?.insertBefore(wrapper, pre)
    wrapper.appendChild(pre)

    const button = document.createElement("button")
    button.type = "button"
    button.className = "code-block-copy-btn"
    button.title = "Copy to clipboard"
    const copyIcon = `<svg viewBox="0 0 16 16" width="14" height="14" fill="currentColor"><path d="M0 6.75C0 5.784.784 5 1.75 5h1.5a.75.75 0 0 1 0 1.5h-1.5a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-1.5a.75.75 0 0 1 1.5 0v1.5A1.75 1.75 0 0 1 9.25 16h-7.5A1.75 1.75 0 0 1 0 14.25Z"></path><path d="M5 1.75C5 .784 5.784 0 6.75 0h7.5C15.216 0 16 .784 16 1.75v7.5A1.75 1.75 0 0 1 14.25 11h-7.5A1.75 1.75 0 0 1 5 9.25Zm1.75-.25a.25.25 0 0 0-.25.25v7.5c0 .138.112.25.25.25h7.5a.25.25 0 0 0 .25-.25v-7.5a.25.25 0 0 0-.25-.25Z"></path></svg>`
    const checkIcon = `<svg viewBox="0 0 16 16" width="14" height="14" fill="currentColor"><path d="M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.751.751 0 0 1 .018-1.042.751.751 0 0 1 1.042-.018L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0Z"></path></svg>`
    button.innerHTML = copyIcon

    button.addEventListener("click", () => {
      const text = code.textContent || ""
      navigator.clipboard.writeText(text).then(() => {
        button.innerHTML = checkIcon
        setTimeout(() => { button.innerHTML = copyIcon }, 2000)
      })
    })

    wrapper.appendChild(button)
  }
}
