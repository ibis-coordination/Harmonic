...
1. **Notification cross-delivery leak.** Per-recipient events (`notifications.delivered`) were dispatched through the collective-audience matcher, sending one member's notification payloads to other members' webhooks. Root cause: event *audience* was implicit — encoded as a fast-path optimization instead of a property of the event type.
...
| **Limits** | rate limits, chain protection, tier gate, max_steps | Sprinkled through dispatch/execution code, with per-rule-type carve-outs (P5) |
...
The per-user notification webhook is a *delivery preference* — "also forward my notifications here" — not a rule the user authors. It has no conditions worth writing, no steps worth choosing, singleton semantics, different billing, different UI. Modeling it as an `AutomationRule` bought reuse of the delivery machinery at the price of permanent carve-outs. It should be explicitly distinguished (at minimum a `rule_type` column; possibly its own table) so that "for every automation rule…" code stops being a trap.
...
### F1. Event-type registry with declared audience
A single registry (likely in/next to `EventService`) declaring every event type and its audience. The dispatcher stops owning `NOTIFICATION_DELIVERED_EVENTS`; per-audience matching branches on the declaration. Emitting an unregistered event type is an error in dev/test.
*Acceptance:* both copies of `NOTIFICATION_DELIVERED_EVENTS` (dispatcher + `AutomationTemplateRenderer`) are gone; all `EventService.record!` call sites (~12) emit registered types; delivery events carry an explicit recipient (P1b).
...
- **Pending rules count toward uniqueness.** The re-keyed index and the one-per-recipient validation now cover URL-less bridge-minted rules — the honest invariant is "one notification-webhook rule per recipient, pending or registered." A redeem race that slips past the pre-mint check surfaces as `ConflictingSetup` (the mint is wrapped). The migration soft-deletes any orphaned pending rule that would collide before building the index.
- **Registered-vs-pending distinction stays Ruby-side.** `#webhook_url` / `#webhook_registered?` read the actions hash; settings pages and the setup-creation validation filter with `detect(&:webhook_registered?)`, preserving the deliberate "pending setups don't block a new setup / don't show as an existing webhook" behavior. No SQL touches the shape.
- **Backfill markers**: `rule_type` from URL shape + live bridge-setup references; `system_managed = TRUE` where the rule sits on a persona agent (`users.system_role`) *and* `created_by_id = ai_agent_id` (the seeder self-signs; human automators sign themselves) — so user-authored rules on persona agents backfill as user-managed.
- **Shape/type consistency is validated both ways**: `notification_webhook` requires a user/agent recipient and no collective; a webhook_url-shaped actions hash on an `automation` rule is rejected (the YAML path can't smuggle a forwarder in).
- **Q5 pruned**: `AutomationRule#path` for forwarder rules now points at the webhook settings page (`/u/:handle/webhook`, `/ai-agents/:handle/webhook`) instead of the phantom user-automations routes.

*Acceptance (all met):* zero raw shape predicates outside migrations ✓; persona deactivate/restore/reseed touches only `system_managed` rules (user rules survive the lifecycle; reseed no longer blocked by user edits) ✓; `rule_type_for_metrics` already renamed to `rule_scope_for_metrics` in F2 ✓.
*Still open from this stage:* whether forwarders eventually migrate to their own table — the column makes that a data migration, not a semantics hunt. Decide when something forces it.
...
- **[decision-semantics-and-action-approval.md](decision-semantics-and-action-approval.md)** — `ProposedAction` (frozen params, `execute_as`, routed through ActionsHelper) is exactly the shape a future "propose" step needs; its three-valued grants (`allowed | with_approval | denied`) are the mechanism Q4 wants for gating automator rule-changes. Its `DecisionResolution` payload work should register through the F1 event-type registry. Its moderation carve-out invariant (moderation actions never gated behind `with_approval`) constrains Q4.
