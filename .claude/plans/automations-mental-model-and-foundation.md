...
*Acceptance:* zero raw `actions->>'webhook_url'` predicates outside the migration; persona deactivate/reseed touches only system-managed rules; the dispatcher's private `rule_type_for_metrics` helper (which returns rule *scope*, agent vs collective) is renamed so `rule_type` has one meaning.
